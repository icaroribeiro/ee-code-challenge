...
import { renewRepository } from '../../../actions/repository';
...
class TagModal extends React.Component {
  ...
  async loadSuggestedTags (repository) {
    try {
      const response = await getRepository(undefined, repository.id, repository)
...
const mapDispatchToProps = dispatch => {
  return {
    onRenewRepository: async (username, id, repository) => {
      try {
        const response = await updateRepository(username, id, repository);
